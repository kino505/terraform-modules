...
mkdir -p ${wd}
export TF_CLI_ARGS_init="-backend-config='bucket=${STATE_BUCKET}' -backend-config='key=network/${APPLICATION}/vpc.tfstate' -backend-config='region=${REGION}'"
terraform -chdir=${wd} init -from-module=git@github.com:kino505/terraform-modules.git//aws/network/vpc?ref=${REF}
