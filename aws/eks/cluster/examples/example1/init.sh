...
mkdir -p ${wd}
export TF_CLI_ARGS_init="-backend-config='bucket=${STATE_BUCKET}' -backend-config='key=eks/${APPLICATION}/cluster.tfstate' -backend-config='region=${REGION}'"
terraform -chdir=${wd} init -from-module=git@github.com:kino505/terraform-modules.git//aws/eks/cluster?ref=${REF}
